...
export function canStartBlockExploration(bx, by, residentIds) {
  ...
  if ((gameState.state.resources.credits || 0) < BALANCE.blockExploration.costCredits) {
    return { ok: false, reason: `星币不足（需要 ${BALANCE.blockExploration.costCredits}）` };
  }
  return { ok: true, block, residentIds: unique };
}
...
export function startBlockExploration(bx, by, residentIds) {
  ...
  gameState.addResource('credits', -BALANCE.blockExploration.costCredits);
  const totalDays = Math.max(3, BALANCE.blockExploration.baseDays - ids.length + 1);

  // 进度条上的随机事件位置（0~1 之间的进度比例，触发后置 fired）
  const events = [];
  for (let i = 0; i < BALANCE.blockExploration.eventCount; i++) {
    events.push({ position: 0.15 + Math.random() * 0.55, fired: false });
  }
  ...
  gameState.addNotification({
    title: '区块探索开始',
    text: `${ids.length} 名居民开始探索区块 (${bx},${by})，预计 ${totalDays} 天（花费 ${BALANCE.blockExploration.costCredits} 星币）。`,
    type: 'info',
    icon: 'compass',
  });
